...
- **macOS** — [`pywhispercpp`](https://github.com/absadiki/pywhispercpp)（whisper.cpp 的 binding），並透過 Core ML encoder 在 Apple Neural Engine 上執行。決策理由：CTranslate2 沒有 Metal/Core ML 路徑，所以會 fallback 到 CPU；同一台 Mac mini 透過 ANE 可以達到 5-7× real-time。
- **Linux** — [`faster-whisper`](https://github.com/SYSTRAN/faster-whisper)（CTranslate2）。為未來的 PVE 部署保留 GPU/CUDA 路徑。
...
> v2 從未對外發佈，所以 v2.1 沒有任何遷移負擔。`registry/models.yaml` schema 是全新的（每個模型一個 variants 列表）。
