...
`make dev-https` 會以 `--ssl-certfile` / `--ssl-keyfile` 指向這些環境變數
來執行 uvicorn。若任一環境變數未設定或指向不存在的檔案,
該 target 會以明確訊息失敗。
...
之後重新啟動 `make dev-https`(或等下次重啟時自然套用新憑證)。
...
| `make dev-https` 出現 "WHISPER_CERT path does not exist" 錯誤 | 憑證檔案被刪除或改名;在 `WHISPER_CERT` 所指向的目錄重新執行 `sudo tailscale cert <host>`。 |
